    ...
    return (
        <>
            <div className="container" style={{color: props.mode==='dark'?'white' :'black'}}>
                ...
                <button disabled={text.length===0} className={`btn btn-${props.mode==='red'?'success':'danger'} m-2`} onClick={handleUpper}>Convert into Uppercase</button>
                <button disabled={text.length===0} className={`btn btn-${props.mode==='red'?'success':'danger'} m-2`} onClick={handleLower}>Convert into Lowercase</button>
                <button disabled={text.length===0} className={`btn btn-${props.mode==='red'?'success':'danger'} m-2`} onClick={handleClear}>Clear Text</button>
                <button disabled={text.length===0} className={`btn btn-${props.mode==='red'?'success':'danger'} m-2`} onClick={RepeatText}>Repeat text</button>
            </div>
